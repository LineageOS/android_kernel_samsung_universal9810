/*
 * max77854-muic.c - MUIC driver for the Maxim 77843
 *
...
#include <linux/muic/max77854-muic-hv.h>
...
struct afc_init_data_s {
	...
	struct max77854_muic_data *muic_data;
};
struct afc_init_data_s afc_init_data;

bool muic_check_is_hv_dev(struct max77854_muic_data *muic_data)
{
	bool ret = false;
...
static int max77854_hv_muic_read_reg(struct i2c_client *i2c, u8 reg, u8 *value)
{
	...
	ret = max77854_read_reg(i2c, reg, value);
	if (ret < 0)
...
static int max77854_hv_muic_write_reg(struct i2c_client *i2c, u8 reg, u8 value)
{
	...
	max77854_read_reg(i2c, reg, &before_val);
	ret = max77854_write_reg(i2c, reg, value);
	max77854_read_reg(i2c, reg, &after_val);
	...
	const u8 reg, const u8 val, const u8 mask, const bool debug_en)
{
	...
	ret = max77854_read_reg(i2c, reg, &before_val);
	if (ret)
		pr_err("%s:%s err read REG(0x%02x) [%d] \n", MUIC_DEV_NAME,
				__func__, reg, ret);
	...
	if (debug_en) {
		ret = max77854_read_reg(i2c, reg, &after_val);
		if (ret < 0)
			pr_err("%s:%s err read REG(0x%02x) [%d]\n",
					MUIC_DEV_NAME, __func__, reg, ret);
...
void max77854_hv_muic_reset_hvcontrol_reg(struct max77854_muic_data *muic_data)
{
...
void max77854_muic_set_afc_ready(struct max77854_muic_data *muic_data, bool value)
{
	bool before, after;
...
static int max77854_hv_muic_state_maintain(struct max77854_muic_data *muic_data)
{
	int ret = 0;
			...
			(struct max77854_muic_data *muic_data)
{
	...
	max77854_hv_muic_read_reg(i2c, MAX77854_MUIC_REG_HVTXBYTE, &hvtxbyte);

	for(i = 0; i < HVRXBYTE_MAX; i++) {
		max77854_hv_muic_read_reg(i2c, (MAX77854_MUIC_REG_HVRXBYTE1+i), &hvrxbyte[i]);
		if(hvrxbyte[i] == 0)
			break;
	}
...
static void max77854_hv_muic_afc_control_ping
		(struct max77854_muic_data *muic_data, bool ping_continue)
{
	int ret;
	...
	if (ping_continue)
		ret = max77854_hv_muic_write_reg(muic_data->i2c, MAX77854_MUIC_REG_HVCONTROL2, 0x5B);
	else
		ret = max77854_hv_muic_write_reg(muic_data->i2c, MAX77854_MUIC_REG_HVCONTROL2, 0x03);
...
static void max77854_hv_muic_qc_charger(struct max77854_muic_data *muic_data)
{
	struct i2c_client	*i2c = muic_data->i2c;
	int ret;
	u8 status3;

	ret = max77854_hv_muic_read_reg(i2c, MAX77854_MUIC_REG_STATUS3, &status3);
	if (ret) {
...
static void max77854_hv_muic_after_qc_prepare(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
...
static void max77854_hv_muic_adcmode_switch
		(struct max77854_muic_data *muic_data, bool always_on)
{
	struct i2c_client	*i2c = muic_data->i2c;
	int ret;
	...
	if (always_on) {
#if !defined(CONFIG_SEC_FACTORY)
		max77854_muic_set_adcmode_always(muic_data);
#endif /* CONFIG_SEC_FACTORY */
...
#if !defined(CONFIG_SEC_FACTORY)
		max77854_muic_set_adcmode_oneshot(muic_data);
#endif /* CONFIG_SEC_FACTORY */
		/* non MAXIM */
		ret = max77854_muic_hv_update_reg(i2c, MAX77854_MUIC_REG_HVCONTROL1,
...
static void max77854_hv_muic_adcmode_always_on(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	max77854_hv_muic_adcmode_switch(muic_data, true);
}

void max77854_hv_muic_adcmode_oneshot(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	max77854_hv_muic_adcmode_switch(muic_data, false);
}

static int max77854_hv_muic_handle_attach
		(struct max77854_muic_data *muic_data, const muic_afc_data_t *new_afc_data)
{
	int ret = 0;
	bool noti = true;
	...
	case FUNC_TA_TO_PREPARE:
		max77854_hv_muic_adcmode_always_on(muic_data);
		max77854_hv_muic_set_afc_after_prepare(muic_data);
	...
	case FUNC_PREPARE_TO_PREPARE_DUPLI:
		...
		muic_data->afc_count++;
		max77854_hv_muic_set_afc_charger_handshaking(muic_data);
		muic_data->is_afc_handshaking = true;
		if (muic_data->afc_count > AFC_CHARGER_WA_PING) {
			max77854_hv_muic_afc_control_ping(muic_data, false);
		} else {
			max77854_hv_muic_afc_control_ping(muic_data, true);
			noti = false;
		}
		break;
	...
	case FUNC_PREPARE_TO_QC_PREPARE:
		...
		max77854_hv_muic_qc_charger(muic_data);
		max77854_hv_muic_after_qc_prepare(muic_data);
		break;
	...
	case FUNC_PREPARE_DUPLI_TO_AFC_9V:
		max77854_hv_muic_afc_control_ping(muic_data, false);
		max77854_hv_muic_adcmode_oneshot(muic_data);
		break;
...
bool muic_check_dev_ta(struct max77854_muic_data *muic_data)
{
...
static void max77854_hv_muic_detect_dev(struct max77854_muic_data *muic_data, int irq)
{
	struct i2c_client *i2c = muic_data->i2c;
	...
	ret = max77854_bulk_read(muic_data->i2c, MAX77854_MUIC_REG_STATUS1, 3, status);
	if (ret) {
	...
	if (intr == MUIC_INTR_ATTACH) {
		...
		ret = max77854_hv_muic_handle_attach(muic_data, tmp_afc_data);
		if (ret)
		...
		ret = max77854_hv_muic_state_maintain(muic_data);
		if (ret)
...
void max77854_muic_prepare_afc_charger(struct max77854_muic_data *muic_data)
{
	struct i2c_client *i2c = muic_data->i2c;
	int ret;
	...
	max77854_muic_set_afc_ready(muic_data, true);
...
static void max77854_hv_muic_detect_after_charger_init(struct work_struct *work)
{
	...
	struct max77854_muic_data *muic_data = init_data->muic_data;
	int ret;
	u8 status3;
	...
	ret = max77854_read_reg(muic_data->i2c, MAX77854_MUIC_REG_STATUS3, &status3);
	if (ret) {
		pr_err("%s:%s fail to read muic reg(%d)\n", MUIC_HV_DEV_NAME,
				__func__, ret);
		return;
	...
	if (muic_data->is_afc_muic_ready) {
		if (muic_data->is_afc_muic_prepare)
			max77854_hv_muic_detect_dev(muic_data, muic_data->irq_vdnmon);
		else
			max77854_hv_muic_detect_dev(muic_data, -1);
	}
...
{
	struct max77854_muic_data *muic_data =
	    container_of(work, struct max77854_muic_data, hv_muic_qc_vb_work.work);
	u8 status3, vbadc;
	...
	max77854_hv_muic_read_reg(muic_data->i2c, MAX77854_MUIC_REG_STATUS3, &status3);
	vbadc = status3 & STATUS3_VBADC_MASK;

	if (vbadc == VBADC_4V_5V || vbadc == VBADC_5V_6V) {
		muic_data->is_qc_vb_settle = true;
		max77854_hv_muic_detect_dev(muic_data, muic_data->irq_vbadc);
	}
...
{
	struct max77854_muic_data *muic_data =
		container_of(work, struct max77854_muic_data, hv_muic_mping_miss_wa.work);
	...
	max77854_hv_muic_detect_dev(muic_data, muic_data->irq_mpnack);
...
void max77854_hv_muic_init_detect(struct max77854_muic_data *muic_data)
{
	int ret;
	u8 status3, vdnmon;
...
void max77854_hv_muic_init_check_dpdnvden(struct max77854_muic_data *muic_data)
{
	u8 hvcontrol1;
	int ret;
	...
	ret = max77854_hv_muic_read_reg(muic_data->i2c, MAX77854_MUIC_REG_HVCONTROL1, &hvcontrol1);
	if (ret) {
...
{
	struct max77854_muic_data *muic_data = data;
	pr_info("%s:%s irq:%d\n", MUIC_HV_DEV_NAME, __func__, irq);
	...
	if (muic_data->is_muic_ready == false)
		...
		max77854_hv_muic_detect_dev(muic_data, irq);
...
int max77854_afc_muic_irq_init(struct max77854_muic_data *muic_data)
{
	int ret = 0;
	...
	if (muic_data->mfd_pdata && (muic_data->mfd_pdata->irq_base > 0)) {
		...
		muic_data->irq_vdnmon = irq_base + MAX77854_MUIC_IRQ_INT3_VDNMON;
		REQUEST_HV_IRQ(muic_data->irq_vdnmon, muic_data, "muic-vdnmon");
		muic_data->irq_mrxrdy = irq_base + MAX77854_MUIC_IRQ_MRXRDY;
		REQUEST_HV_IRQ(muic_data->irq_mrxrdy, muic_data, "muic-mrxrdy");
		muic_data->irq_mpnack = irq_base + MAX77854_MUIC_IRQ_INT3_MPNACK;
		REQUEST_HV_IRQ(muic_data->irq_mpnack, muic_data, "muic-mpnack");
		muic_data->irq_vbadc = irq_base + MAX77854_MUIC_IRQ_INT3_VBADC;
...
void max77854_hv_muic_free_irqs(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
...
int of_max77854_hv_muic_dt(struct max77854_muic_data *muic_data)
{
...
void max77854_hv_muic_initialize(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	...
	INIT_DELAYED_WORK(&muic_data->hv_muic_qc_vb_work, max77854_hv_muic_check_qc_vb);
	INIT_DELAYED_WORK(&muic_data->hv_muic_mping_miss_wa, max77854_hv_muic_check_mping_miss);
}

void max77854_hv_muic_remove(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	...
	max77854_hv_muic_free_irqs(muic_data);
}

void max77854_hv_muic_remove_wo_free_irq(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	cancel_work_sync(&afc_init_data.muic_afc_init_work);
	cancel_delayed_work_sync(&muic_data->hv_muic_qc_vb_work);
