...
#ifdef FP_ENABLE_AP_CONFIG
	/* set cs pin in fp driver, use Exynos8890, 8895 */
				       ...
				       size_t len)
{
	...
	if (len != (unsigned int)len)
		pr_info
		    ("%s vfsspi_writeSync len casting is failed. t.len=%ld, len=%d",
				      ...
				      size_t len)
{
	...
	if (len != (unsigned int)len)
		pr_info
		    ("%s vfsspi_readSync len casting is failed. t.len=%ld, len=%d",
		     __func__, len, (unsigned int)len);
	...
	if (spidev != NULL) {
		...
		if (!vfsspi_device->enabled_clk) {
			...
			if (spi_info != NULL) {
				...
				ret_val =
				    vfsspi_sec_spi_prepare(spi_info, spidev);
				if (ret_val < 0)
...
static int vfsspi_enableIrq(struct vfsspi_device_data *vfsspi_device)
{
	...
	enable_irq(gpio_irq);
	if (vfsspi_device->detect_mode == DETECT_ADM)
...
static void vfsspi_work_func_debug(struct work_struct *work)
{
	...
	vfsspi_device = container_of(work, struct vfsspi_device_data, work_debug);

	pr_info("%s power:%d, irq:%d, tz:%d, type:%s, cnt_irq:%d\n",
		__func__,
		vfsspi_device->ldo_onoff,
		gpio_get_value(vfsspi_device->drdy_pin),
		vfsspi_device->tz_mode,
		sensor_status[vfsspi_device->sensortype + 2],
				vfsspi_device->cnt_irq);
}
...
static int vfsspi_type_check(struct vfsspi_device_data *vfsspi_device)
{
	...
	do {
		...
		spi_sync(spi, &m);
		if (rx_buf[15] == 0x35) {
			vfsspi_device->sensortype = SENSOR_NAMSAN;
			pr_info("%s sensor type is NAMSAN. retry(%d)\n", __func__, retry);
		} else {
			vfsspi_device->sensortype = SENSOR_FAILED;
			pr_info("%s sensor type is not NAMSAN. retry(%d)\n", __func__, retry);
			for (i = 0; i < 16; i++)
				pr_info("%s, %0x\n", __func__, rx_buf[i]);
...
#ifndef ENABLE_SENSORS_FPRINT_SECURE
#ifdef FP_ENABLE_AP_CONFIG
static int vfsspi_set_cs_gpio(struct vfsspi_device_data *vfsspi_device,
			      struct s3c64xx_spi_csinfo *cs)
{
	...
	if (cs->line != 0) {
		status = gpio_request_one(cs->line, GPIOF_OUT_INIT_LOW,
					  dev_name(&vfsspi_device->spi->dev));
		if (status) {
			pr_err("%s Failed to get /CS gpio [%d]: %d\n",
			       __func__, cs->line, status);
		}
	}
	return status;
}
