				   ...
				   int vempty_mode)
{
	...
	if (!fuelgauge->using_temp_compensation) {
		pr_info
		    ("%s: does not use temp compensation, default hw vempty\n", __func__);
	...
	default:
		...
		if (max77705_bulk_write
		    (fuelgauge->i2c, VALRT_THRESHOLD_REG, 2, valrt_data) < 0) {
			pr_info("%s: Failed to write VALRT_THRESHOLD_REG\n", __func__);
			return;
		}
		data = max77705_read_word(fuelgauge->i2c, VALRT_THRESHOLD_REG);
...
static int max77705_fg_read_temp(struct max77705_fuelgauge_data *fuelgauge)
{
	...
	if (max77705_fg_check_battery_present(fuelgauge)) {
		if (max77705_bulk_read(fuelgauge->i2c,
...
static int max77705_fg_read_fullcapnom(struct max77705_fuelgauge_data
				       *fuelgauge)
{
	u8 data[2];
	int ret;
...
static int max77705_fg_read_isys(
	struct max77705_fuelgauge_data *fuelgauge, int unit)
{
	u8 data1[2];
	u32 temp;
	s32 i_current;
	...
	switch (unit) {
	case SEC_BATTERY_CURRENT_UA:
		i_current = temp * 3125 / 10;
		break;
	case SEC_BATTERY_CURRENT_MA:
	default:
		i_current = temp * 3125 / 10000;
	}
...
static int max77705_fg_read_isys_avg(
	struct max77705_fuelgauge_data *fuelgauge, int unit)
{
	u8 data2[2];
	u32 temp;
	...
	switch (unit) {
	case SEC_BATTERY_CURRENT_UA:
		avg_current = temp * 3125 / 10;
		break;
	case SEC_BATTERY_CURRENT_MA:
	default:
		avg_current = temp * 3125 / 10000;
	}
...
static int max77705_fg_read_iin(
	struct max77705_fuelgauge_data *fuelgauge, int unit)
{
	u8 data1[2];
	u32 temp;
...
int max77705_fg_reset_soc(struct max77705_fuelgauge_data *fuelgauge)
{
	...
	pr_info("%s: Before quick-start - current(%d), avg current(%d)\n",
		__func__, max77705_fg_read_current(fuelgauge,
	...
	pr_info("%s: After quick-start - current(%d), avg current(%d)\n",
		__func__, max77705_fg_read_current(fuelgauge, SEC_BATTERY_CURRENT_MA),
...
static int max77705_fg_check_status_reg(struct max77705_fuelgauge_data
					*fuelgauge)
{
	u8 status_data[2];
	int ret = 0;
...
int max77705_fg_alert_init(struct max77705_fuelgauge_data *fuelgauge, int soc)
{
	...
	/* Using RepSOC */
	if (max77705_bulk_read(fuelgauge->i2c, MISCCFG_REG, 2,
			       misccgf_data) < 0) {
	...
	if (max77705_bulk_write(fuelgauge->i2c, MISCCFG_REG,
				2, misccgf_data) < 0) {
	...
	salrt_data[1] = 0xff;
	salrt_data[0] = soc;
	if (max77705_bulk_write(fuelgauge->i2c, SALRT_THRESHOLD_REG,
				2, salrt_data) < 0) {
	...
	talrt_data[1] = 0x7F;
	talrt_data[0] = 0x80;
	if (max77705_bulk_write(fuelgauge->i2c, TALRT_THRESHOLD_REG,
				2, talrt_data) < 0) {
	...
	read_data =
	    max77705_read_word(fuelgauge->i2c, TALRT_THRESHOLD_REG);
	if (read_data != 0x7f80)
	...
	max77705_update_reg(fuelgauge->pmic,
...
bool max77705_fg_init(struct max77705_fuelgauge_data *fuelgauge)
{
	...
	if (fuelgauge->pdata->jig_gpio) {
		...
		pr_info("%s: jig_result : %d\n", __func__,
			max77705_check_jig_status(fuelgauge));
	...
	if (fuelgauge->pdata->thermal_source != SEC_BATTERY_THERMAL_SOURCE_FG) {
		...
		if (max77705_bulk_write(fuelgauge->i2c, CONFIG_REG,
					2, data) < 0) {
			pr_info("%s : Failed to write CONFIG_REG\n", __func__);
			return false;
		}
			       ...
			       int enable)
{
	...
	if ((status_data[1] & 0x01) && !lpcharge && !fuelgauge->is_charging) {
		pr_info
		    ("%s : Battery Voltage is Very Low!! SW V EMPTY ENABLE\n", __func__);
...
static void max77705_fg_get_scaled_capacity(struct max77705_fuelgauge_data
					    *fuelgauge,
					    union power_supply_propval *val)
{
	...
	psy_do_property("battery", get, POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT,
			swelling_val);
#endif

	if (get_power_supply_by_name("max77705-charger")) {
		chg_val.intval = cable_val.intval;
		psy_do_property("max77705-charger", get, POWER_SUPPLY_PROP_CURRENT_AVG,
				chg_val);
		psy_do_property("max77705-charger", get, POWER_SUPPLY_PROP_CHARGE_NOW,
				chg_val2);
	} else {
		chg_val.intval = 0;
		chg_val2.strval = "NONE";
	}

	if (is_hv_wireless_type(fuelgauge->cable_type)
	    || is_hv_wire_type(fuelgauge->cable_type))
	...
	if ((cable_val.intval != SEC_BATTERY_CABLE_NONE) &&
		...
		curr = max77705_get_fuelgauge_value(fuelgauge, FG_CURRENT_AVG);
		...
		pr_info("%s : curr(%d) topoff(%d) capacity_max(%d)\n", __func__,
			curr, topoff, max_temp);

		if ((curr < capacity_threshold) && (curr > topoff)) {
			if (!cnt) {
				cnt = 1;
				fuelgauge->standard_capacity =
				    (val->intval <
				     fuelgauge->pdata->capacity_min) ? 0 : ((val->intval -
					fuelgauge->pdata->capacity_min) * 999 /
				...
				temp = (val->intval <
					...
					fuelgauge->pdata->capacity_min));
				sample =
				    ((capacity_threshold - curr) * (999 - fuelgauge->standard_capacity))
				    / (capacity_threshold - topoff);

				pr_info
				    ("%s : %d = ((%d - %d) * (999 - %d)) / (%d - %d)\n",
				     __func__, sample, capacity_threshold, curr,
				     fuelgauge->standard_capacity,
				     capacity_threshold, topoff);
...
static int max77705_fg_calculate_dynamic_scale(
	struct max77705_fuelgauge_data *fuelgauge, int capacity)
{
	union power_supply_propval raw_soc_val;

	raw_soc_val.intval = max77705_get_fuelgauge_value(fuelgauge,
							  FG_RAW_SOC) / 10;
	...
	if (raw_soc_val.intval <
		...
		fuelgauge->capacity_max =
			max77705_fg_check_capacity_max(fuelgauge,
			fuelgauge->capacity_max);
...
static void max77705_fg_check_qrtable(struct max77705_fuelgauge_data *fuelgauge)
{
	u16 data;

	data = max77705_read_word(fuelgauge->i2c, QRTABLE20_REG);
	if (data != fuelgauge->battery_data->QResidual20) {
		if (max77705_write_word(fuelgauge->i2c, QRTABLE20_REG,
					fuelgauge->battery_data->QResidual20) < 0)
			pr_err("%s: Failed to write QRTABLE20\n", __func__);
	}

	data = max77705_read_word(fuelgauge->i2c, QRTABLE30_REG);
	if (data != fuelgauge->battery_data->QResidual30) {
		if (max77705_write_word(fuelgauge->i2c, QRTABLE30_REG,
				    ...
				    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		if (fuelgauge->pdata->capacity_calculation_type &
		    SEC_FUELGAUGE_CAPACITY_TYPE_DYNAMIC_SCALE)
			max77705_fg_calculate_dynamic_scale(fuelgauge, val->intval);
		break;
#if defined(CONFIG_EN_OOPS)
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
		...
		fuelgauge->capacity_max = max77705_fg_check_capacity_max(fuelgauge, val->intval);
		fuelgauge->initial_update_of_soc = true;
		break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		switch (val->intval) {
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		{
			...
			if (max77705_write_word
			    (fuelgauge->i2c, FULLSOCTHR_REG, reg_fullsocthr) < 0) {
				       ...
				       *fuelgauge)
{
	struct device_node *np =
	    of_find_node_by_name(NULL, "max77705-fuelgauge");
	...
	if (np == NULL) {
		...
		if (fuelgauge->using_hw_vempty) {
			...
			ret = of_property_read_u32(np,
						"fuelgauge,sw_v_empty_voltage_cisd",
						&fuelgauge->battery_data->sw_v_empty_vol_cisd);
			if (ret < 0) {
				pr_err
				    ("%s error reading sw_v_empty_default_vol_cise %d\n",
				     __func__, ret);
				fuelgauge->battery_data->sw_v_empty_vol_cisd = 3100;
			}

			ret = of_property_read_u32(np,
						"fuelgauge,sw_v_empty_recover_voltage",
						&fuelgauge->battery_data->sw_v_empty_recover_vol);
			if (ret < 0)
				pr_err
				    ("%s error reading sw_v_empty_recover_vol %d\n",
				     __func__, ret);

			pr_info
			    ("%s : SW V Empty (%d)mV,  SW V Empty recover (%d)mV\n",
		...
		ret =
		    of_property_read_u32(np, "full_check_current_2nd",
...
static int max77705_fuelgauge_probe(struct platform_device *pdev)
{
	...
	fuelgauge_data =
	    kzalloc(sizeof(sec_fuelgauge_platform_data_t), GFP_KERNEL);
	if (!fuelgauge_data) {
		ret = -ENOMEM;
		goto err_free;
	}
	...
	raw_soc_val.intval =
	    max77705_get_fuelgauge_value(fuelgauge, FG_RAW_SOC) / 10;
